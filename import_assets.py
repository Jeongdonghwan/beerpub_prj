...
def build_files(src_dir, out_dir, prefix, max_px, thumb_px=None):
    """원본이 있으면 변환, 없으면(서버) 기존 출력 폴더 스캔. 파일명 리스트 반환."""
    out_dir.mkdir(parents=True, exist_ok=True)
    if src_dir.exists():
        for i, f in enumerate(sorted(src_dir.glob("*.png")) + sorted(src_dir.glob("*.jpg")), 1):
            kb = optimize(f, out_dir / f"{prefix}_{i:02d}.webp", max_px)
            line = f"  {prefix}_{i:02d}.webp {kb}KB"
            if thumb_px:
...
def run():
    ...
    reviews = build_files(SRC_REVIEWS, OUT_REVIEWS, "rv", 720)
